 ...
 * NOTE:
 *
 * This file is assumed to follow a fixed format to allow easy parsing
...
// Allow a worker context to run any ready task
// on top of the current execution stack when a task blocks
// to help with global progress. MAY CAUSE DEADLOCKS!
#define HCLIB_WORKER_OPTIONS_HELP_GLOBAL  0x01

// Allow a worker context to run a ready task from
// the current finish scope on top of the current
// execution stack when blocking on an end-finish
// to help with that finish scope's progress.
